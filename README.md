...
The main goal of this lab exercise is for you and your teammates to develop a proficiency in using the Kanban board as a way to facilitate teamwork, and provide proof of having engaged in teamwork productively using the Kanban board.  You must work collaboratively via the cards on your Kanban board, and of course GitHub, assigning the various tasks to different people on your team, ensuring that everyone on the team has at least two tasks assigned to them (if you need to come up with fairly small tasks to come up with two per team member that's OK - small tasks are actually good!)
...
## Process
You are expected to create card on the board for tasks that need to be completed, and move those cards through the various pipelines on the board as they are processed.  Each card must include the URLs of the ALL the commits that related to process on that card's task.  These links will added to the "Description" textbox of the card as URL links.  In addition to the link (which is required) some brief descriptive text may be added as well.  The *primary* documentation of the purpose of each commit must be the commit comment, but having a summary description on the card can aid in the usefulness of the board as an aid to effective teamwork.

In addition to using the Kanban board to add cards for the various tasks needed to complete this exercise, suitably assigned to individual team members and moved from one pipeline to the next until complete, make sure you employ good practices: TDD using Criterion, git branching, code coverage with transparent testing, etc.  If there are bugs in your code, document them with Criterion tests as well.
